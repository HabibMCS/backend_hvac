...
def read_transformed_df():
    ...
    sys_df_dict = {}
    temp_sys_log = pd.read_csv(f'.\Temporary\\tempo_unique_sys.csv')
        
    for i in temp_sys_log['System Identifier']:
        sys_df_dict[f'sys_{i}_df'] = pd.read_csv(f'.\Temporary\\sys_{i}_df.csv')
...
#***********************************************************************************************************************#
    m_comp1 = comp1_mech_power / ((h_comp1_dis - h_comp_suc)+0.00001)  ### V0.03 modified
    m_comp2 = comp2_mech_power / ((h_comp2_dis - h_comp_suc)+0.00001)  ### V0.03 modified
#***********************************************************************************************************************#
...
def remove_unecessary_files():
    temp_sys = pd.read_csv(f'.\Temporary\\tempo_unique_sys.csv')
    tot_sys = pd.read_csv(f'.\Clean transformed data before calculation\\unique_sys.csv')
    ...
    for i in missing_sys['System Identifier']:
        if os.path.exists(f'.\Results with outliers\\results.{i}.csv'):
              os.remove(f'.\Results with outliers\\results.{i}.csv')
...
def remover_outliers(sys_df):
    for col in ['COP','cooling_load (KW)']:
        ...
        lower_outliers = sys_df[sys_df[col] < lower_fence][col].values
        upper_outliers = sys_df[sys_df[col] > upper_fence][col].values
...
### V0.03 added check file exist function to accumulate results in Folder(Results without outliers)
#***********************************************************************************************************************#               
...
def perform_calculations(sys_df,z):
    ...
    print(sys_df['COP'].isna().sum())
    sys_df.dropna(axis=0, how='any', inplace=True)
    print(z,sys_df.shape)
    # Saving the final dataframe in csv
    sys_df.to_csv(f'.\Temporary\\results.{z}.csv', index=False)
    sys_df.to_csv(f'.\Results with outliers\\results.{z}.csv', index=False)
    sys_df[:] = remover_outliers(sys_df)
    ### V0.03 modified how the results are saved to accumulate results in Folder(Results without outliers)
#***********************************************************************************************************************#               
    check_file_exist(sys_df, f'.\Results without outliers\\results.{z}.csv',data_df=True)

#***********************************************************************************************************************# 
#     sys_df.to_csv(f'.\Results without outliers\\results.{z}.csv', index=False)
